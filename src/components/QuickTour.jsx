 ...
 * 각 단계는 메뉴를 실제로 전환하고, data-tour 앵커를 스포트라이트로 비춘다. 데모 학급(2026ai)에서 가장 잘 보인다.
 */
export const TOUR_STEPS = [
  ...
  { menu: '기록', target: 'menu-서류함', title: '⑨ 서류함 — 학교 제출용', body: '학운위 심의 서식·보호자 동의 안내 .docx(한글 호환), 전자 동의 링크와 제출 현황, 도입 절차 4단계 점검. 학교 절차는 여기서 끝납니다. 체험 끝! 데모 학급 2026ai에서 직접 눌러 보세요.' },
];
...
const QuickTour = ({ open, onClose, setActiveMenu }) => {
  ...
  useEffect(() => {
    if (!open) return;
    const onKey = (e) => { if (e.key === 'Escape') onClose(); if (e.key === 'ArrowRight') setIdx(i => Math.min(TOUR_STEPS.length - 1, i + 1)); if (e.key === 'ArrowLeft') setIdx(i => Math.max(0, i - 1)); };
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 3000 }}>
      ...
      {rect ? (
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(15, 23, 42, 0.62)' }} onClick={onClose} />
      )}
      ...
      <div className="glass-card" style={{ position: 'absolute', width: cardW, background: 'white', borderRadius: '18px', padding: '18px 20px', boxShadow: '0 20px 50px rgba(0,0,0,0.35)', ...cardStyle, transition: 'top 0.3s, left 0.3s' }} onClick={e => e.stopPropagation()}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
          ...
          <button onClick={onClose} title="닫기 (Esc)" style={{ marginLeft: 'auto', background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--text-faint)', display: 'flex' }}><X size={18} /></button>
        </div>
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <button className="btn btn-secondary" style={{ padding: '6px 10px', fontSize: '0.82rem' }} onClick={onClose}>건너뛰기</button>
          <div style={{ marginLeft: 'auto', display: 'flex', gap: '6px' }}>
            ...
            <button className="btn btn-primary" style={{ padding: '6px 12px', fontSize: '0.82rem' }} onClick={() => (last ? onClose() : setIdx(i => i + 1))}>{last ? '체험 끝내기' : <>다음 <ChevronRight size={14} /></>}</button>
          </div>
        </div>
      </div>
    </div>
